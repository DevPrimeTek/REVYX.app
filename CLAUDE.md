...
*CLAUDE.md · v1.2.40 · 2026-07 · CONFIDENȚIAL · Uz Intern*
...
| Versiune | Data | Note |
|---|---|---|
| **1.2.40** | **2026-07** | ★ PATCH — **Demo responsive tablet/mobile — navigare + zero overflow orizontal (Samsung S22 + Xiaomi Pad).** Senior Full Stack Dev (bugfix `apps/web-preview`, demo freeze parțial). Trigger fondator: „ajustează demo la tablete și mobil". Fix critic: bara nav era `hidden lg:flex` → sub 1024px zero navigare + 221px overflow la 1024px (item M2 vechi). Cod: (1) `site-nav.tsx` NEW `MobileDrawer` (hamburger + drawer glisant, clip layer anti-overflow, body-scroll lock, ESC/backdrop close, toate destinațiile grupate) + breakpoint `lg`→`xl` (bara desktop doar ≥1280px, rezolvă M2 la 1024). (2) `ui/button.tsx` `whitespace-nowrap` (fără suprapunere text la etichete lungi). (3) overflow orizontal eliminat chirurgical (`flex-wrap`/`flex-col sm:flex-row`/`min-w-0`): 12 header-e card, grupuri buton (dashboard/deals[id]/escalations), filtre (leads/tasks/settings), task-list, input-uri add, rând notificare. (4) i18n `nav.menu`/`nav.closeMenu` × RO/RU/EN. Tests: typecheck PASS · lint PASS (1 OD-01) · build PASS 26 routes + /api/feedback · Playwright live @360/800/1024/1279/1280/1440 × RO/RU/EN = 0px overflow pe toate 26 rutele + drawer funcțional. Backend + Master Plan + BRD INTACTE (Regula 8 + 10 + 12 + 18 + 21). |
| **1.2.39** | **2026-07** | ★ PATCH — **Demo feedback: formular NATIV (stilul demo) + bază de date Supabase, înlocuiește Tally.** Director Marketing + FRONTEND WEB DEV + DOC. Trigger fondator: „forma să fie în stilul demo + stocată într-o bază de date". Cod `apps/web-preview/`: NEW `components/feedback/feedback-form.tsx` (Modal brand + rating 1-5 + textarea RO/RU/EN) + NEW `app/api/feedback/route.ts` (insert Supabase Postgres via REST, cheie service_role server-only) + NEW `lib/feedback-payload.ts` (validare partajată); rescris `feedback-widget.tsx` (deschide formularul nativ) + `feedback-config.ts` + i18n `feedback.*` extins × 3 limbi. Docs: `FEEDBACK_FORM_TALLY.md` → `FEEDBACK_DB_SETUP.md` v1.1.0 (SQL + env Vercel + export/GDPR) · README kit v1.1.0 + EXECUTION_PLAN sync. Tests: typecheck PASS (1 TS5101) · lint PASS (1 OD-01) · build PASS 26 routes + /api/feedback · i18n 0 duplicate · smoke 400/503/randare OK. INDEX v1.1.30 → **v1.1.31**. Backend `apps/api/` + Master Plan + BRD INTACTE (Regula 6 + 8 + 11 + 12 + 18 + 21). |
