...
| Versiune | Data | Note |
|---|---|---|
| **1.2.38** | **2026-07** | ★ PATCH — **DOC CONSISTENCY FIX — 3 fix-uri MED post-revizie documentație (docs-only).** Senior Architect + DOC. Trigger: revizie completă documentație cerută de PM (pregătire bootstrap ProFixer). Fixate: (1) §6 Phase 0 checklist bifat + header `⛔ BLOCANT` → `✅ COMPLETE (M1.S1)` — era desincronizat cu §0a (Phase 0 închisă la M1.S1); item Privacy/Cookie anotat cu status real (drafts livrate, legal review pending). (2) §3 Auth: `Supabase Auth sau Auth0` (propunere pre-dev) → ★ DECIS M1.S1 AuthService propriu (NestJS + jose + Argon2id, JWT RS256) + cross-ref TECH_SPEC phase0-security. (3) `docs/skills/DOC_MASTER.md` v1.0.0 → **v1.0.1** — cross-ref mort `BRD_REVYX_v1.1.0.md` → `BRD_REVYX_v1.5.0.md` (§1 + §3.1). Aparițiile `BRD_v1.1.0` din docs istorice (HST/audit/spec-uri închise) neatinse intenționat (point-in-time records). Zero cod modificat; backend + apps + Master Plan + BRD INTACTE (Regula 4 + 8 + 18 + 21). |
