...
<!-- BRD_REVYX_v1.5.0.md · v1.5.0 · 2026-06 -->
...
| **1.5.0** | **2026-06** | ★ Solution Architect + Senior PM + Senior BA | ★ **MINOR — BR-32 Partner Registry Governance.** Registry-ul de parteneri de cooperare (folosit la publicarea MLS — BR-29) primește guvernanță explicită pe `account_type ∈ {agency, individual}`: cont **agency** → lista partajată, editabilă doar de `team_lead`/`manager`/`admin` (agenții = read-only la selectare); cont **individual** → lista personală editabilă de owner. `CooperationPanel` consumă lista rezolvată (`resolve_partners(account)`) și permite doar selectare. Adăugări: §6.5 BR-32 + notă guvernanță în §18.10. Pur aditiv — zero modificare BR-01..BR-31, formule §7, entități §8. Trigger: PM (combinație variante agency + individual cu reguli stricte de scriere). |
...
| ★ **BR-32** | **Partner Registry Governance:** Registry-ul de parteneri de cooperare (folosit la publicarea MLS — BR-29) are **proprietar și reguli de scriere determinate de tipul contului** (`account_type ∈ {agency, individual}`). (a) **Cont agency** — lista aparține agenției (partajată de toți agenții); **doar `team_lead` / `manager` / `admin`** pot adăuga/edita/șterge parteneri; `agent` + `senior_agent` au acces **read-only** și o folosesc doar la selectare în `CooperationPanel`. (b) **Cont individual** — lista e personală; agentul-proprietar are drept complet de scriere. Rezolvarea listei la publicare: `resolve_partners(account)` → lista agenției SAU lista personală. Niciun agent nu adaugă parteneri din ecranul de publicare (doar selectare). Producție: `account_type` derivat din tenant (solo = individual; ≥2 agenți = agency) + `user_role` din RBAC §10. | NAR Art. 3 cooperare + APAIM + guvernanță RBAC §10 | RIDICAT |
...
- ★ **Partner Registry (BR-32):** rețeaua de parteneri către care se publică NU se definește din ecranul proprietății — există un **registry dedicat** cu guvernanță pe `account_type`. Pentru conturi **agency**, lista e partajată și o administrează doar `team_lead`/`manager`/`admin` (cabinet agenție → tab „Parteneri"); agenții o folosesc read-only la selectare. Pentru conturi **individual**, agentul își administrează propria listă (cabinet agent → tab „Parteneri"). `CooperationPanel` consumă lista rezolvată (`resolve_partners(account)`) și permite doar selectarea partenerilor + cota de comision + Open House. Entitate producție: `partners` (FK `tenant_id` + `scope` + `default_split_pct`).
...
*BRD_REVYX_v1.5.0.md · v1.5.0 · 2026-06 · CONFIDENȚIAL · Uz Intern*
