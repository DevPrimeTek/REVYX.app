...
**Trigger v1.0.3:** ★ **M0.S3 ✅ CLOSED — Web Static Demo livrat**. Output: (1) `apps/web-preview/` promovat → `apps/web/` (rename git mv + package name `@revyx/web@0.2.0` + workflow rename `web-ci.yml`). (2) Mock data full: 100 leads (LS distribution HOT/qualified/warm/nurturing 12/22/36/30%), 50 properties (apartament/casă/teren/comercial mixed cu PS+LF realistic), 20 deals (distribuție 6 stages), 8 agenți cu APS/Trust/Slots. (3) i18n RO/RU/EN via custom React context provider + `messages/{ro,ru,en}.json` (~120 keys/limbă) + language switcher în `<SiteNav>` cu persistență localStorage. (4) `/deals` drag-drop @dnd-kit (PointerSensor distance 6 + KeyboardSensor + DragOverlay rotire +1°) cu click-to-advance permanent ca a11y fallback. (5) 3 pagini noi: `/settings` (tenant config), `/profile` (agent profile + APS 6-month bar chart), `/notifications` (audit-log feed). (6) Build pass `next build` 16 routes (15 static + 1 dynamic `/leads/[id]`). T-M0.S3-14 DNS rămâne PARTIAL (vercel.json deployed M0 deploy pipeline; custom domain `demo.revyx.app` documented step în Runbook v1.0.2).
...
|---|---|---|---|---|---|
| ★ T-M0.S3-01 | ☑ | Promote `apps/web-preview/` → `apps/web/` + rename CI workflow | FRONTEND WEB DEV + DEVOPS | XS | git mv (33 fișiere) + `package.json` rename `@revyx/web@0.2.0` + `.github/workflows/web-ci.yml` paths bumped |
