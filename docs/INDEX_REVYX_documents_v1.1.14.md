...
| **1.1.14** | **2026-05** | ★ Senior Architect + Senior PM + DOC | ★ **PATCH — Documentation consolidation (Regula 18 introduction).** Sesiune docs-only triggered de PM directive post-PR #36. Schimbări: (a) **55 fișiere predecesoare șterse din `docs/`** — Roadmap v1.0.0..v1.0.8 (9 files) + INDEX v1.0.0..v1.1.12 (20 files) + Master Plan v1.1.1 + BRD v1.0.0 + READINESS_phase5 v1.0.0..v1.0.5 (6 files) + DPIA_phase5 v1.0.0 + SCC_VENDORS_phase5 v1.0.0..v1.0.1 (2 files) + RUNBOOK_demo-deploy v1.0.1 + RUNBOOK_partition-maintenance v1.0.0 + TECH_SPEC predecessors (audit-log 2 + churn-ga 2 + marketplace 1 + match-engine 1 + ml-pricing-ga 3 + mobile-rn 1 + tenancy-roles 1) + 2 legal duplicates RO (`cookie-policy.md` + `privacy-policy.md`). Doar ultima versiune din fiecare familie e păstrată. (b) **Cross-refs din toate docs supraviețuitoare actualizate via sed bulk** — 17 mapări (vezi commit). Niciun broken cross-ref rămas. (c) Roadmap rename v1.0.9 → **v1.0.10** (Edit + git mv pattern). (d) INDEX rename v1.1.13 → **v1.1.14**. (e) CLAUDE.md bump v1.2.16 → **v1.2.17** cu ★ NEW Regula 18 în §10b — "Single living document, no version proliferation". Doc bump = Edit (nu Write) + git mv la noul filename; niciodată 2 versiuni paralele simultan. Migrare retroactivă executată o singură dată. (f) Toate docs supraviețuitoare păstrate INTACTE (conținut nemodificat — doar cross-refs actualizate). Backwards compat full cu v1.1.13 (zero modificări `apps/api/` + `apps/web-preview/` + framework specs). Trigger: PM directive "consolidați informația în fișiere și ștergeți toate fișierele vechi din documentație; vreau să fie o regulă să nu creați fișiere noi". |
