...
<!-- INDEX_REVYX_documents_v1.1.4.md · v1.1.4 · 2026-05 -->
...
| **1.1.4** | **2026-05** | ★ DEVOPS + FRONTEND WEB DEV + Senior PM + DOC | ★ **PATCH — M0 deploy pipeline online (Vercel GitHub App + CI build gate).** Bring-forward task T-M0.S3-14 (Vercel setup) înainte de M0.S3 substantive work pentru visibility PM live pe M0.S2 prototype. Adăugare 1 runbook NEW (★ `docs/runbook/RUNBOOK_REVYX_demo-deploy_v1.0.0.md` — Vercel project creation steps + preview URL flow per PR + Ignored Build Step recipe + security headers + troubleshooting) + 1 workflow CI NEW (`.github/workflows/web-preview-ci.yml` — typecheck + build gate paralel pe orice PR cu modificări în `apps/web-preview/**` sau `design/tokens.json`) + 1 config NEW (`apps/web-preview/vercel.json` — framework Next.js + install/build commands + region fra1 + 4 security headers). Custom domain `demo.revyx.app` rămâne tracked la M0.S3 T-M0.S3-14 (DNS setup). Backwards compat full cu v1.1.3 (zero modificări cod prototype, doar additive infra). Trigger: PM request "visual test + CI/CD pentru deploy automat" + Regula 6 INDEX update. |
...
### 6.x Deploy & CI/CD (★ NEW v1.1.4)

| Artefact | Descriere |
|---|---|
| ★ `docs/runbook/RUNBOOK_REVYX_demo-deploy_v1.0.0.md` v1.0.0 | NEW M0 (early bring-forward T-M0.S3-14 partial) — Vercel deploy procedure pentru `apps/web-preview/`. §2 one-time setup PM (Vercel project creation + Root Directory = `apps/web-preview` + Ignored Build Step recipe). §3 operational flow (preview URL per PR, production deploy pe main). §4 troubleshooting (5 simptome comune). §5 security headers default (X-Frame-Options DENY + 3 alte). Custom domain `demo.revyx.app` tracked separat T-M0.S3-14 (DNS). Owner: DEVOPS + FRONTEND WEB DEV + Senior PM. |
| ★ `.github/workflows/web-preview-ci.yml` | NEW M0 — CI build gate paralel; rulează `npm ci → typecheck → next build` la fiecare PR cu modificări în `apps/web-preview/**` sau `design/tokens.json`. Node 22 + npm cache. Owner: DEVOPS + FRONTEND WEB DEV. |
| ★ `apps/web-preview/vercel.json` | NEW M0 — Vercel project config: framework Next.js, install/build commands, region `fra1` (low-latency Moldova), 4 security headers default. Schema-validated. Owner: DEVOPS. |

---

## 12. Document count summary (★ deploy-pipeline M0, v1.1.4)

| Categorie | Count |
|---|---|
| Foundation (CLAUDE.md + brand) | 2 (CLAUDE.md ★ v1.2.6) |
| Strategic Planning | 4 (MASTER_PLAN v1.0.0 [history] + v1.1.0 [history] + v1.1.1 [history] + v1.1.2 active) |
| Platform & Detailed Roadmap | 4 (Roadmap v1.0.2 ACTIVE; v1.0.1 [HISTORY], v1.0.0 [HISTORY-partial]) |
| Design System (§2c) | 3 (tokens.json + screens-inventory + apps/web-preview ★ + vercel.json) |
| BRD | 2 |
| Tech Specs (Phase 0-4 + Phase 5 + Frontend/Design) | 36 |
| Workflows | 11 |
| Runbooks | 12 (★ +1: demo-deploy v1.0.0) |
| Audit + Readiness | 19 |
...
| **Total** | **116 documente** (★ +1 vs v1.1.3 = demo-deploy runbook; CLAUDE.md bump v1.2.5 → v1.2.6 in-place; `.github/workflows/web-preview-ci.yml` + `apps/web-preview/vercel.json` infra additive, contate ca artefacte design-system §6.x). |

> **Notă:** documente duplicate prin versiuni listate separat. **Active** post-deploy-pipeline: **~94** (+1 runbook activ vs M0.S2).
