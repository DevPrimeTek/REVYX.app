...
### ★ 4.5 — Practici de teren + Etică profesională (NAR/APAIM) [ETH/MLS/BNA/LPD]

> **Sursă:** BRD v1.4.0 §18.9/§18.10/§18.11 + `TECH_SPEC_REVYX_realtor-ethics_v1.0.0` + `TECH_SPEC_REVYX_mls-cooperation_v1.0.0`. Extinde AGI Layer (§4.4) cu practici operaționale de teren și conformitate etică NAR/APAIM.

**M1.S3 — tasks (în paralel cu Phase B + AGI M1.S3):**

| Task | Status | Titlu | Hats | Effort | Dep | Detaliu |
|---|---|---|---|---|---|---|
| ★ **T-BNA-01** | ⬜ | **Buyer Needs Assessment — entitate + completeness gate (BR-31)** | BACKEND DEV + DBA | M | T-MD-01 (§17.1 buget) | Migration `0026_buyer_assessments.sql` din BRD §8.5.1 cu `assessment_completeness` GENERATED. Service: BR-31 — dacă `< 0.50` AND LS ≥ 0.60 → create task `complete_buyer_assessment` (priority MEDIUM). Câmpuri alimentează Match Engine + FRS. AC-AGI-12. |
| ★ **T-ETH-01** | ⬜ | **Ethics Checkpoints — extindere 4→6 trigger-uri (BR-28)** | BACKEND DEV + SECURITY | S | T-AGI-01 (entitate) | Extindere enum `trigger_context` cu `exclusive_listing_solicitation` (Art.16) + `misleading_advertising` (Art.12) + câmp `nar_article`. Trigger Art.16: la asignare agent pe lead cu `buyer_assessments.prior_agent_agreement IS NOT NULL`. Per `TECH_SPEC_REVYX_realtor-ethics` §5. AC-AGI-13. |

**M1.S4 — tasks (în paralel cu Match + NBA + AGI M1.S4):**

| Task | Status | Titlu | Hats | Effort | Dep | Detaliu |
|---|---|---|---|---|---|---|
| ★ **T-MLS-01** | ⬜ | **MLS Cooperation — entitate + gate mandat (BR-29)** | BACKEND DEV + DBA + Compliance | L | T-MD-04 (mandat) | Migration `0027_cooperation_offers.sql` din BRD §8.5.1. Gate publish: `mandate_status='signed'`. State machine open→engaged→closed/withdrawn. Cron `mls_mandate_guard` (mandat expirat → withdrawn). Per `TECH_SPEC_REVYX_mls-cooperation` §4-§5. AC-AGI-09. |
| ★ **T-MLS-02** | ⬜ | **Commission Split Engine + AUDIT append-only** | BACKEND DEV | M | T-MLS-01 ✅ | La deal CÂȘTIGAT cu `cooperating_agent_id` → split (sale: %preț · rent: 1× chirie) × `offered_split_pct`, logat append-only + `MLS_COMMISSION_SPLIT_LOGGED`. Idempotent. Spec §6. AC-AGI-10. |
| ★ **T-MLS-03** | ⬜ | **Open House / ДОД — extensie SHOWING + raportare scrisă proprietar** | BACKEND DEV + FRONTEND WEB DEV | M | T-MLS-01 ✅ | SHOWING +2 câmpuri (`is_open_house` + `invited_partner_agent_ids`). Raport săptămânal proprietar (alimentează PKI). UI tab „Cooperare (MLS)" pe property detail + queue „Oferte de cooperare". Spec §7+§11. |
| ★ **T-MLS-04** | ⬜ | **MLS inter-tenant config + cross-tenant view PII-masked** | BACKEND DEV + SECURITY | M | T-MLS-01 ✅ | Config admin `tenant.mls_inter_agency_enabled` + view `public_mls` cu PII proprietar mascat (analog marketplace). RBAC §10. |
| ★ **T-LPD-01** | ⬜ | **Listing Price Discipline — overpricing_risk + LF impact (BR-30)** | BACKEND DEV + ML ENGINEER | S | pricing AI hooks ✅ | `overpricing_risk` computed (`list_price > AI_suggested × 1.15`). LF degradare accelerată + trigger etic `misleading_advertising`. UI card avertisment + script ACM. `LISTING_OVERPRICING_FLAGGED`. AC-AGI-11. |

**Dependențe inter-task:**
- T-ETH-01 depinde de T-AGI-01 (entitatea `ethics_checkpoints`) + T-BNA-01 (`prior_agent_agreement`).
- T-LPD-01 trigger `misleading_advertising` se leagă de T-ETH-01.
- T-MLS-01 depinde de T-MD-04 (mandat exclusiv §17.5).
- Toate T-MLS-02..04 depind de T-MLS-01.
