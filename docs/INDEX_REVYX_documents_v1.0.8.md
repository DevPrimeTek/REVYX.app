...
<!-- INDEX_REVYX_documents_v1.0.8.md · v1.0.8 · 2026-06 -->
...
| 1.0.8 | 2026-06 | Senior PM + Audit Lead | ★ PATCH — post-Stage 4 launch (T+77 close) + S16 audit pass · adăugare 4 documente S17: (`AUDIT_REVYX_s16-external-pass` v1.0.0 audit Stage 4 exit gates 7/7 PASS + BR-18 RLS 6/6 + outcome flow time-skip + F-S14-01 CLOSED FULL + F-S15-01/02/03 CLOSED + F-S16-01 LOW new finding · `RUNBOOK_REVYX_stage5-white-label-launch` v1.0.0 operational day-by-day Stage 5 first Enterprise tenant + Platform Matrix §14 Modul 13 cross-ref · `READINESS_REVYX_phase5` v1.0.5 PATCH cu exit gates Stage 4 measured + Stage 5 entry 17/17 🟢 GREEN · `TECH_SPEC_REVYX_ml-pricing-ga` v1.0.4 PATCH F-S15-01 closed spec-side bias sample tightening 30→50) · re-marcaj `★` pe entries S17; entries S16 mențin marker în istoric · bump count summary §12 (96 → 100 documente totale post-S17) · cross-ref nou §0.19 Stage 5 runbook entry sign-off + §7 Stage 5 entry · F-S14-01 MED CLOSED FULL (CMS deploy T+62) · F-S15-01 MED CLOSED spec-side (ml-pricing-ga v1.0.4) · F-S15-02 LOW CLOSED operational (DevOps infra-tune T+68) · F-S15-03 LOW CLOSED spec-side (migrare 0613 + cron tune) · F-S14-03 LOW CLOSED spec-side (migrare 0545) · F-S16-01 LOW new finding TRACKED (NTP skew cron observability) · F-S14-02/04 + F-S11-07 TRACKED unchanged |
...
- Sortare per categorie → alfabetică/cronologică. `★` înaintea numelui = adăugat/actualizat la sesiunea curentă (S17 acum).
...
| ★ `docs/MASTER_PLAN_REVYX_execution-roadmap_v1.1.1.md` | PATCH S15-bis-3 — Trio canonical introduction. §0 Status Tracker ★ S17 update (S16 ✅ → S17 ✅; next S18). Backwards compat full cu v1.1.0. Versiunea **activă**. |
...
| `TECH_SPEC_REVYX_white-label_v1.0.0.md` | Custom domain per tenant Enterprise, DKIM, edge HMAC. Data model `tenant_domain` + `white_label_config` + `tenant_email_domain` + `tenant_wl_audit`. 12 WL_* events. **S17: spec referință Stage 5 launch** (RUNBOOK_stage5-white-label-launch v1.0.0). Owner: DevOps + Security Lead. |
...
| ★ `TECH_SPEC_REVYX_ml-pricing-ga_v1.0.4.md` | NEW S17 — PATCH **F-S15-01 MED CLOSED spec-side**. `min_sample_district_n: 30 → 50` config flag bump pentru evita borderline bias alerts pe districte rurale cu sample mic pre-Stage 5 GA. DPIA §5.3 cross-ref update minor planificat v1.0.1 PATCH S18. Backwards compat full (config JSON; no code; instant rollout fără retraining). NU breach Art. 22 — compliance best-practice tightening. Cross-ref AUDIT_s15 §3 + §5.2 + AUDIT_s16 §5.2 + READINESS v1.0.5 §7.1 gate 5.16 (deployed T+76). Owner: DS Lead + DPO + Senior Compliance Auditor + Senior Architect. |
...
| `TECH_SPEC_REVYX_audit-log_v1.1.1.md` | PATCH — closes F-S10-09 + F-S11-02. Total 79 events Phase 5. S15-S17 verificat: BUYER_* 13 (Stage 2) + PRICING_MODEL_* (Stage 3) + CHURN_* 14 (Stage 4) + WL_* 12 (Stage 5 pre-flight) emise conform; assertNoPII PASS 100%. Owner: Senior Security Auditor. |
| `TECH_SPEC_REVYX_tenancy-roles-extension_v1.1.0.md` | MINOR — closes F-05 + F-06. 6 roluri custom Phase 5 (cs_user, cs_lead, tenant_admin, compliance_auditor, data_science_lead, etc). View `audit_log_compliance_view`. Migrare 0610. **S17: §4.5 cs_user/cs_lead RLS BR-18 verified 6/6 PASS × 2 cicluri**. Owner: Security Lead + Senior DBA. |
...
| `docs/runbook/RUNBOOK_REVYX_dkim-rotation_v1.0.0.md` | DKIM 90-day rotation cycle + DMARC. **S17: staging rehearsal PASS T+72** (selector `rvx20260714` test cycle + rollback simulated); Stage 5 dependency CLOSED. Owner: Security Lead + DevOps. |
...
| `docs/runbook/RUNBOOK_REVYX_stage4-churn-launch_v1.0.0.md` | Operational day-by-day Stage 4 (T+56 → T+77) Churn pilot CS dry-run intern. **S17: executat CLOSED PASS T+77 cu 7/7 exit gates** (task SLA 9/9 · 0 PII leak · BR-18 RLS 6/6 · AUC drift 0.9% · outcome flow time-skip · tri-lingual playbook adoption 3/3 · CHECKLIST 100%). Owner: DS Lead + CS Lead + Solution Architect + Senior QA + Security Lead + Backend Lead + DPO + Audit Lead + VP Product. |
| ★ `docs/runbook/RUNBOOK_REVYX_stage5-white-label-launch_v1.0.0.md` | NEW S17 — operational day-by-day Stage 5 (T+77 → T+91) **White-Label first Enterprise tenant**. Pre-flight T+76 entry gates (17 items: Stage 4 exit + DKIM staging + DPA + DNS + Cloudflare HMAC + Let's Encrypt + WL_* 12 events + DMARC report-only 7d + BSI DPA + 4 migrări bundle + ml-pricing-ga v1.0.4 + warm pool tune). Sequence T+77 entry / T+78 domain claim + TLS / T+79-80 brand+DKIM / T+84 first rotation rehearsal / T+88 plan-tier downgrade test / T+91 exit + GA decision. Daily health 10 metrici. Rollback decision tree 5 branches (DMARC fail / TLS fail / edge HMAC mismatch / plan-tier downgrade bug / tenant NPS). Tenant cohort = 1 Enterprise pilot real (TENANT-ENT-PILOT-01). **Platform Matrix §14 Modul 13 cross-ref (White-Label 🌐 Web only complet DP-05 — 5/5 features admin-only Web)**. Cross-ref phase5-rollout-sequence §8 + white-label v1.0.0 + dkim-rotation §3-§9 + audit-log §4.4.3 (WL_* 12 events) + AUDIT_s16 §8 + SCC v1.0.1 §3.6 (BSI signed T+77). Owner: DevOps + Security Lead + Backend Lead + DPO + Audit Lead + VP Product + CTO + CISO + Sales Lead + Legal. |
...
| ★ `docs/audit/AUDIT_REVYX_s16-external-pass_v1.0.0.md` | NEW S17 — audit pass post Phase 5 Stage 4 (Churn pilot CS dry-run intern) execuție T+56..T+77. Verifică **7/7 exit gates** §6.2 READINESS PASS (task SLA 9/9 closed 0 EXPIRED · 0 PII leak · BR-18 RLS 6/6 × 2 cicluri · AUC drift 0.9% · outcome flow PREVENTED→RETAINED time-skip PASS · tri-lingual playbook adoption 3/3 · CHECKLIST 100%). §3 BR-18 RLS test E2E report 6 scenarii × 2 cicluri (12 test runs) + DP-05 enforcement Mobile blocked. §4 outcome flow time-skip 90d test report. F-S14-01 MED CLOSED FULL (CMS T+62). F-S15-01 MED CLOSED spec-side (ml-pricing-ga v1.0.4). F-S15-02 LOW CLOSED operational (DevOps infra-tune T+68). F-S15-03 + F-S14-03 LOW CLOSED spec-side (migrare 0613 + 0545). F-S16-01 LOW new finding (NTP skew cron observability) TRACKED. **Stage 5 entry gates 17/17 GREEN** (incl. BSI DPA signed T+77 BSI-M4 + 4 migrări bundle deployed T+75 + ml-pricing-ga v1.0.4 + warm pool tune + DKIM staging rehearsal). Cross-ref READINESS v1.0.5 + RUNBOOK stage5-white-label-launch + ml-pricing-ga v1.0.4. Owner: Audit Lead + echipa virtuală 7-rol. |
...
| ★ `docs/audit/READINESS_REVYX_phase5_v1.0.5.md` | NEW S17 PATCH — post-Stage 4 (T+77 close) + pre-Stage 5. §6.1 Stage 4 entry gates 4.1-4.9 toate ☑ 2026-06-22. §6.2 exit gates **7 metrici măsurate (7/7 PASS)**. §7.1 Stage 5 entry gates **5.1-5.17 toate 🟢 GREEN** (Stage 4 exit ✅ + DKIM staging T+72 + Tenant Enterprise DPA T+74 + tenant_admin T+75 + DNS T+75 + Cloudflare HMAC T+73 + Let's Encrypt T+75 + WL_* 12 events smoke T+74 + DMARC report-only 7d + pii_field_registry 84 rows + BSI DPA signed T+77 BSI-M4 + bundle 0611+0612+0545+0613 deployed T+75 + ml-pricing-ga v1.0.4 deployed T+76 + warm pool/cron tune T+68+T+75). §0.19 Stage 5 runbook approved. §10 cross-ref nou AUDIT_s16 + stage5-white-label-launch + ml-pricing-ga v1.0.4. Owner: Audit Lead + Senior PM. |
...
| `docs/cs-playbooks/CHURN_MEDIUM_v1.1.0.md` | MINOR — closes F-S10-06. RU + EN templates paralele. **S17: Stage 4 dry-run validated tri-lingual adoption (CSU-01 RO outcome INTERVENED_SUCCESS T+58)**. Owner: CS Lead + Senior PM. |
| `docs/cs-playbooks/CHURN_HIGH_v1.1.0.md` | MINOR — closes F-S10-06. RU + EN paralele. **S17: Stage 4 dry-run validated (CSU-02 RU outcome INTERVENED_SUCCESS T+64)**. Owner: CS Lead + Senior PM. |
| `docs/cs-playbooks/CHURN_CRITICAL_v1.1.0.md` | MINOR — closes F-S10-06. RU + EN paralele. **S17: Stage 4 dry-run CRITICAL injection T+70 simulated CLOSE successfully (cs_lead intervention)**. Owner: CS Lead + Senior PM. |
...
| `docs/legal/DPIA_REVYX_phase5_v1.0.0.md` | DPIA Phase 5 single-source. Sign-off triple DPO + Security Lead + CISO §10. **S17: §5.4 churn-ga Art. 22 + balancing test verified Stage 4 (1 human override invocation T+68); §5.5 white-label data flows referenced pre-Stage 5**. F-S14-04 LOW verbiage clarification + F-S15-01 §5.3 cross-ref update minor → v1.0.1 PATCH planificat S18 (T-S18-03). Owner: DPO + Security Lead + CISO. |
...
| `docs/legal/SCC_VENDORS_phase5_v1.0.1.md` | S15 PATCH — closes F-S13-03 LOW. Apple+Google ON FILE + BSI §3.6 expandat pre-Stage 5 (BSI-M1..M4 plan). **S17: BSI-M4 milestone completat T+77 (DPA dual-signed)**; SCC v1.0.2 PATCH planificat S18 (T-S18-04). Owner: DPO + Legal Lead + Senior Compliance Auditor + CISO + Audit Lead. |
...
## 12. Document count summary (S17 close, v1.0.8)
...
| BRD | 2 |
| Tech Specs (Phase 0-4 + Phase 5) | 35 (★ +1: ml-pricing-ga v1.0.4) |
| Workflows | 11 |
| Runbooks | 11 (★ +1: stage5-white-label-launch v1.0.0) |
| Audit + Readiness | 14 (★ +2: AUDIT_s16 v1.0.0 + READINESS v1.0.5) |
| CS Playbooks + Checklists | 7 |
| Legal | 5 |
| Test fixtures + Templates | 3 |
| Skills | 5 |
| **Total** | **100 documente** (incl. patch + minor versions; +4 vs v1.0.7 = 1 AUDIT + 1 RUNBOOK + 1 READINESS + 1 PATCH spec) |

> **Notă:** documente duplicate prin versiuni sunt listate separat — fiecare versiune e document distinct istoric. Total **active** (deprecate excluse, MASTER_PLAN v1.0.0+v1.1.0 supersedat de v1.1.1): **~83** post-S17.
...
4. Refresh manual la fiecare audit checkpoint (post-S12, S13, S14, S15, S16, S17 — acum, S18+).
...
*docs/INDEX_REVYX_documents_v1.0.8.md · v1.0.8 · 2026-06 · CONFIDENȚIAL · Uz Intern*
