...
<!-- INDEX_REVYX_documents_v1.0.2.md · v1.0.2 · 2026-05 -->
...
- Sortare per categorie → alfabetică/cronologică. `★` înaintea numelui = adăugat/actualizat la sesiunea curentă (S14 acum).
...
| Document | Descriere |
|---|---|
| `TECH_SPEC_REVYX_mobile-rn_v1.0.0.md` | React Native mobile app — push notifications, OT auth, MOBILE_DEVICE_*/AUTH_MOBILE_OT_* events. Zero PII validare via @revyx/test-fixtures-pii. Stage 1 rollout 8 săpt cu 30 agenți pilot. ★ S14: executat closed PASS T+14 cu 9/9 exit gates (NPS +34, crash-free 99.42%, push delivery 97.1%). Owner: Mobile Lead + Solution Architect. |
| `TECH_SPEC_REVYX_marketplace-two-sided_v1.0.0.md` | Buyer self-service profile + contact-grant flow cu rate-limiting. Acoperă entity BUYER_PROFILE (BRD §8.3) + GDPR consent flow. Initial v1.0.0; bump v1.0.1 inclus în S10 cross-spec. ★ S14: gating Stage 2 entry (T+14 GO sync). Owner: Backend Lead + DPO. |
...
| `TECH_SPEC_REVYX_audit-log_v1.1.1.md` | PATCH — closes F-S10-09 (CHURN_CS_TASK_OPENED alerting clarifying) + F-S11-02 (familia §4.4.9 PHASE5_* cu 4 events oficiali). Total 79 events Phase 5. Cross-ref phase5-rollout-sequence stage events. ★ S14: verificat emitere conform în Stage 1 cu cumulative 168 OT issued + 3614 push sent. Owner: Senior Security Auditor. |
...
| ★ `docs/runbook/RUNBOOK_REVYX_stage2-marketplace-launch_v1.0.0.md` | NEW S14 — operational day-by-day Stage 2 (T+14 → T+35) Marketplace pilot. Acoperă pre-flight T+13 entry gates (10 items incl. Stripe live + DPIA §5.2 + Privacy Policy buyer-side), sequence T+14/T+15/T+16-17/T+18-27/T+28/T+29-34/T+35, daily health check Senior QA (12 metrici), rollback decision tree expandat (6 branches — PII leak / contact abuse / spam / Stripe desync / panic / GDPR regression), audit events expected (13 events), tenant pilot selection criteria. Cross-ref phase5-rollout-sequence §5 + marketplace-two-sided §16 + scc-vendors §3.5 + DPIA §5.2. Owner: Marketplace Lead (Backend Lead) + Senior QA + CS Lead + DPO + Security Lead + Audit Lead. |
...
| ★ `docs/audit/AUDIT_REVYX_s13-external-pass_v1.0.0.md` | NEW S14 — audit pass post Phase 5 Stage 1 (Mobile TestFlight) execuție T+0..T+14. Verifică 9/9 exit gates §3.2 READINESS PASS (crash-free 99.42% · push delivery 97.1% · OT success 98.7% · NPS +34 · 0 PII regression · 0 bug critic). F-S11-03 (SCC Apple/Google) OPERATIONAL FULL CLOSED post-signature 2026-04-29 + 2026-05-02; F-S11-08 (4-eyes E2E) rămâne TRACKED gating Stage 3 T+35. Cohort 49/50 installed (98%), audit events cumulative aliniate (168 OT issued, 3614 push, zero auth anomaly). 4 noi findings F-S13-01..04 (1 MED UX deep-link backlog + 3 LOW retention/SCC/Sentry tuning) — toate TRACKED, NU blochează Stage 2 entry. Cross-ref `READINESS_REVYX_phase5` v1.0.2 + `RUNBOOK_REVYX_stage2-marketplace-launch` v1.0.0. Owner: Audit Lead + echipa virtuală 7-rol. |
...
| ★ `docs/audit/READINESS_REVYX_phase5_v1.0.2.md` | PATCH S14 — post-Stage 1 launch (T+14 close) + pre-Stage 2 entry. §2 Pre-flight gate 0.14 trecut 🟢 ON FILE (Apple 2026-04-29 + Google 2026-05-02 signed); §3.1 entry gates 1.1-1.10 toate ☑ cu data 2026-05-04; §3.2 exit gates **valori măsurate** 9/9 PASS (crash-free 99.42% etc.); §4.1 Stage 2 entry 2.1/2.5/2.8 🟢 GREEN (Stage 1 ✅ + designer hand-off + Stripe live), 2.2/2.3/2.4/2.6/2.7 🟡 PENDING DEPLOY/CONTRACT pre-T+14 GO; §4.2 noi exit metrici Stripe webhook lag + rate-limit alert; §0.16 nou gate Stage 2 runbook approved; §10 cross-ref `AUDIT_REVYX_s13-external-pass` + `RUNBOOK_REVYX_stage2-marketplace-launch`. Owner: Audit Lead + Senior PM. |
...
| `docs/legal/SCC_VENDORS_phase5_v1.0.0.md` | Register central SCC pentru toate procesoarele US-based + non-EU implicate Phase 5: Apple APNS §3.1 · Google FCM §3.2 · Cloudflare §3.3 · AWS eu-west-1 §3.4 · Stripe §3.5 · BSI Group MD §3.6. Plan operațional T-14→T-0 §5 + DPO checklist §6 + re-review trimestrial §7. ★ S14: status §3.1+§3.2 OPERATIONAL ON FILE post 2026-04-29 + 2026-05-02 signatures (doc-side PATCH v1.0.1 programat S15 — F-S13-03 tracked). Cross-ref DPIA §3.1 Art. 44+ + READINESS §0.14. Owner: DPO + Legal Lead + CISO + Senior Compliance Auditor. |
...
## 12. Document count summary (S14 close)
...
| Workflows | 9 |
| Runbooks | 8 (★ +1 stage2-marketplace-launch) |
| Audit + Readiness | 8 (★ +2: AUDIT_s13 + READINESS v1.0.2) |
| CS Playbooks + Checklists | 7 |
| Legal | 4 |
| Test fixtures + Templates | 3 |
| Skills | 5 |
| **Total** | **79 documente** (incl. patch + minor versions) |

> **Notă:** documente duplicate prin versiuni (v1.0.0 + v1.1.0 + v1.0.1) sunt listate separat — fiecare versiune e document distinct istoric. Total **active** (deprecate excluse): **~65** post-S14.
...
4. Refresh manual la fiecare audit checkpoint (post-S12, S13, S14 — acum, S15, etc.).
...
*docs/INDEX_REVYX_documents_v1.0.2.md · v1.0.2 · 2026-05 · CONFIDENȚIAL · Uz Intern*
