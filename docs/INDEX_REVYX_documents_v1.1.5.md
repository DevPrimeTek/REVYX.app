...
| **1.1.5** | **2026-05** | ★ FRONTEND WEB DEV + DEVOPS + DESIGNER + ARCHITECT + DOC | ★ **PATCH — M0.S3 ✅ CLOSED Web Static Demo (in-place semantic upgrade).** Adăugări (Regula 10 nouă — physical directory `apps/web-preview/` retained pentru Vercel deploy stability; un attempt anterior cu git mv → `apps/web-preview/` a fost rolled back când a rupt deploy-ul): (a) `apps/web-preview/package.json` upgrade in-place la `@revyx/web-preview@0.2.0` (b) NEW PATCH `docs/ROADMAP_REVYX_detailed-execution_v1.0.3.md` — §3.3 T-M0.S3-01..13 marcate ☑ (T-M0.S3-01 amended) (c) NEW Runbook `docs/runbook/RUNBOOK_REVYX_demo-deploy_v1.0.2.md` — §1 Regula 10 rule + §2.3 DNS step `demo.revyx.app` + §4 9-row troubleshooting (d) NEW dir `apps/web-preview/lib/mock/` (7 fișiere: types, rng deterministic xmur3+sfc32, agents 8, leads 100, properties 50, deals 20, index barrel) (e) NEW dir `apps/web-preview/messages/` (ro/ru/en ~120 keys each) (f) NEW `apps/web-preview/components/i18n/provider.tsx` (g) NEW `apps/web-preview/components/deals/kanban-board.tsx` (h) NEW 3 pages `apps/web-preview/app/{settings,profile,notifications}/page.tsx` (i) NEW `apps/web-preview/.eslintrc.json` pentru lint non-interactive. **Tests primary + secondary executate** (Regula 10): typecheck PASS, lint PASS (1 warning OD-01 known font), build PASS 16 routes (15 static + 1 dynamic), dev smoke test 14/14 routes HTTP 200 + DOM verified 100 leads + 50 props + 20 deals + 20 drag handles + lang switcher + 404 fallback. CLAUDE.md bump v1.2.6 → v1.2.8 (v1.2.7 retragat ca first-attempt with rename rollback). Trio canonical: Master Plan v1.1.2 + Platform Matrix v1.0.0 + Roadmap **v1.0.3** ACTIVE. Trigger: PM feedback "deploymentul nu este trecut Build Failed Root Directory apps/web-preview does not exist" + Regulile 4 + 6 + 8 + ★ **10 NEW**. |
...
| ★ `docs/ROADMAP_REVYX_detailed-execution_v1.0.3.md` | NEW M0.S3 — **PATCH M0.S3 ✅ CLOSED**. §3.3 T-M0.S3-01..13 marcaje ☑ + T-M0.S3-14 ◐ (DNS step PARTIAL). T-M0.S3-01 amended: physical directory `apps/web-preview/` retained per Regula 10 (semantic upgrade in-place fără git mv). Mock data 100/50/20, i18n RO/RU/EN custom provider, drag-drop @dnd-kit, 3 pagini noi. §0 trigger note + build evidence 16 routes + tests primary/secondary output. Backwards compat full cu v1.0.2. Versiunea **activă**. Owner: FRONTEND WEB DEV + DEVOPS + DESIGNER (Creative Director) + ARCHITECT + Senior PM + DOC. |
...
| Design System (§2c) | 7 (tokens.json + screens-inventory + ★ apps/web-preview/ promoted + ★ apps/web-preview/lib/mock/ + ★ apps/web-preview/messages/ + ★ apps/web-preview/components/i18n/ + ★ apps/web-preview/components/deals/) |
| BRD | 2 |
...
| **Total** | **120 documente** (★ +4 vs v1.1.4 = Roadmap v1.0.3 PATCH + Runbook demo-deploy v1.0.2 PATCH + INDEX v1.1.5 + design-system §2c artefacte M0.S3 in-place adds (mock/messages/i18n/deals kanban/.eslintrc) rolled into a single category bump; CLAUDE.md bump v1.2.6 → v1.2.8 in-place adăugând Regula 10). |
...
9. ★ Post-M0.S3 close: PATCH bump v1.1.5 reflectă Web Static Demo livrat (Roadmap v1.0.3 + Runbook v1.0.2 + apps/web-preview/ promote din apps/web-preview/ + 5 §2c artefacte mock/i18n/dnd-kit + CLAUDE.md bump v1.2.7).
