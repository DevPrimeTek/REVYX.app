...
<!-- ROADMAP_REVYX_detailed-execution_v1.0.1.md · v1.0.1 · 2026-05 -->
...
**Trigger v1.0.1:** Creative Director shift (S20 close → M0.S1 entry) — Figma traditional path înlocuit cu **direct-to-code**. Tokens devin single source of truth pentru tailwind.config (zero handoff Figma→cod); FRONTEND WEB DEV hat activat early în M0.S1 alături de DESIGNER + ARCHITECT.
...
| **1.0.1** | **2026-05** | ★ DESIGNER (Creative Director) + Senior Architect + Frontend Lead + Senior PM + DOC | **PATCH — direct-to-code shift M0.S1 + M0.S3 scope rebalance.** §3.1 M0.S1: T-M0.S1-03..08 reformulate de la wireframe Figma → page stub React + componente shadcn customized; T-M0.S1-09 (`design/tokens.json`) păstrat ca livrabil principal; T-M0.S1-10 (component primitives) re-deplasat în `apps/web-preview/components/ui/`. §3.3 M0.S3: T-M0.S3-01..03 marked PARTIAL inherited din M0.S1 (skeleton + tailwind + primitives livrate early); T-M0.S3-04..14 își păstrează scope original (mock data 100/50/20 + content full + i18n + Vercel deploy). NU breaking change pentru AC-M0-01..07 sau pentru M1/M2 sub-stages. Backwards compat: orice referință la "T-M0.S1-03..08 wireframe" pre-PATCH se re-citește ca "T-M0.S1-03..08 page stub direct-to-code". Cross-ref `TECH_SPEC_REVYX_ui-design-system_v1.0.0.md` § (NEW) + `design/tokens.json` (NEW). |

> **Compat (v1.0.0 → v1.0.1):** Niciun task din Pre-dev, M1.S1..S8, M2.S1..S8 NU e modificat. Doar M0.S1 (10 tasks) reorientate ca *route* + M0.S3 (3 tasks marked PARTIAL inherited). AC-M0-01..07 sunt **neschimbate**; output-ul M0.S1 atinge `≥ 12 ecrane navigabile` deja la M0.S2 close (vs original "wireframe-uite") — net positive vs plan.
...
### 3.2 M0.S2 — Clickable Prototype (Direct-to-code flows)

> Pre-existent neschimbat — exceptând input source: în loc de Figma prototypes, M0.S2 leagă page stubs M0.S1 prin `<Link href>` cu hover/active states deja prezente. Tasks de mai jos rămân valide cu output `apps/web-preview/app/**/page.tsx` (in-place enhancement).

| ID | Task | Owner | Effort | DEP | Platform | Output |
|---|---|---|---|---|---|---|
| T-M0.S2-01 | Click-through Lead intake → Score → Assign (J1) | DESIGNER + FRONTEND WEB DEV | M | T-M0.S1-10 | 🌐 | nav links + interactive states |
| T-M0.S2-02 | Click-through Property creation → Match (J2) | DESIGNER + FRONTEND WEB DEV | M | T-M0.S1-10 | 🌐 | flow |
| T-M0.S2-03 | Click-through Deal pipeline → close-won (J3) | DESIGNER + FRONTEND WEB DEV | M | T-M0.S1-10 | 🌐 | flow + drag-drop M0.S3 stub |
| T-M0.S2-04 | Click-through Manager escalation queue (J4) | DESIGNER + FRONTEND WEB DEV | M | T-M0.S1-10 | 🌐 | flow |
